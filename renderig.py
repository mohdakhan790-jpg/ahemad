...
CHECK_INTERVAL = 1.5               # small sleep inside per-thread watcher to avoid busy-loop
THREAD_FETCH_INTERVAL = 150        # 90 sec
WELCOME_MESSAGE = "welcome to Ahmad bot type help to see available commands ⚡"
MAX_MESSAGES_TRACK = 200
MAX_HISTORY_MESSAGES = 10  # max user+assistant pairs

GC_WELCOMES = [
    "🎉🍁AHMAD KHAN WELCOMES YOU 🌷🌹",
    " 🎀ASSALAMUALAIKUM AND NAMASTE🌸 ",
    " ✨ WELCOME KESE HO TUM ✨"
]
...
def get_ai_response(thread_id: str, sender_id: str, query: str, cl: Client) -> str:
    ...
    with history_lock:
        ...
        if not messages or messages[0]['role'] != 'system':
            messages.insert(0, {
                'role': 'system',
                'content': "You are Ahmad's helpful bot named Ahmad-Bot. Respond helpfully and conversationally."
            })
